...
const StatCard = ({ icon: Icon, label, value, subtext, color = "blue", large = false }: { icon?: any; label: string; value: React.ReactNode; subtext?: React.ReactNode; color?: string; large?: boolean }) => {
  const c = STAT_PALETTE[color] || STAT_PALETTE.blue;
  return (
    <div className="analytics-stat-card" data-color={color} style={{ background: c.bg, border: `1px solid ${c.border}` }}>
      <div style={{ position: "absolute", top: 0, right: 0, width: "4px", height: "100%", background: c.accent }} />
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: large ? "10px" : "6px" }}>
        {Icon ? <div style={{ padding: large ? "8px" : "5px", background: c.iconBg, color: c.accent, borderRadius: "10px", display: "flex" }}><Icon size={large ? 20 : 15} /></div> : <span />}
        <p style={{ margin: 0, color: c.text, fontWeight: 900, fontSize: large ? "15px" : "11px" }}>{label}</p>
      </div>
      <h3 style={{ margin: "2px 0 0", color: c.accent, fontSize: large ? "29px" : "18px", fontWeight: 900, lineHeight: 1.1 }}>{value}</h3>
  ...
  return (
    <div className="space-y-3" dir="rtl">
      ...
      <div className="analytics-kpi-grid">
        <StatCard icon={DollarSign} label="إجمالي الموازنة" value={`${budgetTotal.toLocaleString("en-US")} ج`} subtext={`${budgetRows.length} صنف`} color="teal" />
        <StatCard icon={Building2} label="إجمالي الأصول الجديدة" value={`${assetsTotal.toLocaleString("en-US")} ج`} subtext={`${assetsNewRows.length} أصل`} color="emerald" />
        <StatCard icon={TrendingUp} label="توقع العام القادم (+30%)" value={`${forecastNextYear.toLocaleString("en-US")} ج`} subtext={`+${forecastIncrease.toLocaleString("en-US")} ج زيادة`} color="blue" />
        <StatCard icon={Target} label="متوسط تكلفة الصنف" value={`${avgItemCost.toLocaleString("en-US")} ج`} subtext={`ذروة: ${peakMonth?.name || "-"}`} color="green" />
      </div>
...
const AdminReportsInsights = ({ adminReports }: { adminReports: any[] }) => {
  ...
  return (
      <section className="analytics-panel" dir="rtl">
      {/* Header */}
      <div className="analytics-panel__header">
        <h3 className="analytics-panel__title">
          <CalendarDays size={16} /> تحليلات التقارير الإدارية
        </h3>
        <span className="analytics-panel__badge">
          {adminReports.length.toLocaleString("en-US")} سجل
        </span>
      </div>

      {/* KPI Cards */}
      <div className="analytics-kpi-grid analytics-kpi-grid--reports">
        <StatCard icon={DollarSign} label="إجمالي المصروفات" value={`${totalValue.toLocaleString("en-US", { maximumFractionDigits:0 })} ج`} subtext={`${adminReports.length} سجل`} color="teal" />
        <StatCard icon={Package} label="إجمالي الكميات" value={totalQty.toLocaleString("en-US")} subtext="وحدة مصروفة" color="amber" />
        <StatCard icon={Building2} label="عدد المخازن" value={uniqueStores} subtext={`${uniqueTasks} مهمة مختلفة`} color="violet" />
        <StatCard icon={CalendarDays} label="ذروة الصرف" value={peakMonth?.fullName || "-"} subtext={peakMonth ? `${peakMonth.value.toLocaleString("en-US")} ج` : "لا بيانات"} color="blue" />
      </div>
...
const PurchaseRequisitionsSection = React.memo(({ supabase, currentUser, logAction, showToast, setConfirmDialog, tabDataCache }: any) => {
  ...
  const [purchaseSortField, setPurchaseSortField] = useState("request");
  ...
  const normalizeStatus = (status: any) => {
    ...
    if (s.includes("in review") || s.includes("inreview") || s.includes("under review") || ["review", "قيد المراجعة", "قيد المراجعه", "قيد التنفيذ", "pending", "قيد الانتظار"].includes(s)) return "review";
  ...
  const sortValue = (r: any, field: string, items = false) => {
    if (field === "request") return Number((items ? itemRequestNo(r) : requestNo(r)).replace(/[^0-9.-]/g, "")) || 0;
    const v = r[field] ?? (field === "description" ? getDescription(r) : "");
    return typeof v === "number" ? v : String(v).toLowerCase();
  };
  const sortPurchaseRows = (rows: any[], items = false) => [...rows].sort((a, b) => {
    const av = sortValue(a, purchaseSortField, items), bv = sortValue(b, purchaseSortField, items);
    const cmp = typeof av === "number" && typeof bv === "number" ? av - bv : String(av).localeCompare(String(bv), undefined, { numeric: true });
    return purchaseSortDir === "asc" ? cmp : -cmp;
  });
  const togglePurchaseSort = (field: string) => { if (purchaseSortField === field) setPurchaseSortDir(d => d === "asc" ? "desc" : "asc"); else { setPurchaseSortField(field); setPurchaseSortDir("asc"); } };
  const sortArrow = (field: string) => purchaseSortField === field ? (purchaseSortDir === "asc" ? "↑" : "↓") : "⇅";
  ...
  const filteredSummary = useMemo(() => sortPurchaseRows(uniqueSummaryRows.filter(r => {
  ...
  }), false), [uniqueSummaryRows, search, statusFilter, purchaseSortField, purchaseSortDir]);
  const visibleItems = useMemo(() => sortPurchaseRows(uniqueItemRows.filter(r => {
  ...
  }), true), [uniqueItemRows, selectedRequest, purchaseSortField, purchaseSortDir]);
  ...
  const card = (label: string, value: number, color: string, icon: any, filter: string = "all") => (
    <button
      ...
      style={{ padding: 0, border: statusFilter === filter ? "3px solid #1e1b4b" : "2px solid #cbd5e1", borderRadius: "15px", background: "white", cursor: "pointer", textAlign: "right", boxShadow: statusFilter === filter ? "0 8px 20px rgba(49,46,129,.22)" : "0 4px 12px rgba(15,23,42,.08)", transition: "all .2s", overflow: "hidden" }}
      title={`عرض الطلبات: ${label}`}
    >
      <div style={{ pointerEvents: "none" }}><StatCard icon={icon} label={label} value={englishToArabic(value)} color={color} large /></div>
    </button>
  );

  if (manageSchema) return <div dir="rtl" style={{ fontFamily: "Cairo, sans-serif" }}>
    <button onClick={() => setManageSchema(null)} style={{ marginBottom: "14px", border: "2px solid #1d4ed8", background: "linear-gradient(135deg,#2563eb,#1e40af)", color: "white", borderRadius: "12px", padding: "12px 18px", cursor: "pointer", fontWeight: 900, fontSize: "14px", boxShadow: "0 6px 16px rgba(37,99,235,.25)" }}>← العودة إلى لوحة طلبات الشراء</button>
  ...
  return <div dir="rtl" style={{ fontFamily: "Cairo, sans-serif" }}>
    <div style={{ background: "linear-gradient(135deg,#312e81,#4f46e5 55%,#0f766e)", color: "white", borderRadius: "18px", padding: "20px", marginBottom: "14px", boxShadow: "0 8px 25px rgba(79,70,229,.18)" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: "12px", flexWrap: "wrap" }}>
        ...
        <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
          <button onClick={() => setManageSchema("summary")} style={{ border: "2px solid #bfdbfe", background: "#2563eb", color: "white", borderRadius: "11px", padding: "11px 15px", fontWeight: 900, fontSize: "13px", cursor: "pointer", boxShadow: "0 4px 12px rgba(30,64,175,.3)" }}>✚ إدارة الطلبات</button>
          <button onClick={() => setManageSchema("purchases")} style={{ border: "2px solid #a7f3d0", background: "#059669", color: "white", borderRadius: "11px", padding: "11px 15px", fontWeight: 900, fontSize: "13px", cursor: "pointer", boxShadow: "0 4px 12px rgba(5,150,105,.3)" }}>✎ إدارة الأصناف</button>
          <button onClick={load} style={{ border: "2px solid #fde68a", background: "#d97706", color: "white", borderRadius: "11px", padding: "11px 15px", fontWeight: 900, fontSize: "13px", cursor: "pointer", boxShadow: "0 4px 12px rgba(217,119,6,.3)" }}>↻ تحديث البيانات</button>
        </div>
      </div>
    </div>
    ...
    {view === "summary" ? <>
      ...
      <div style={{ background: "white", border: "1px solid #e2e8f0", borderRadius: "12px", overflow: "auto" }}><table style={{ width: "100%", borderCollapse: "collapse", minWidth: "850px", fontSize: "12px", border: "1px solid #94a3b8" }}><thead style={{ background: "#0f766e", color: "white" }}><tr>{[["رقم الطلب","request"],["البند","item_name"],["الوحدة","unit"],["الكمية المطلوبة","quantity_requested"],["الكمية المنفذة","quantity_executed"],["المتبقي","quantity_remaining"],["القسم الطالب","requesting_department"],["الحالة","executor"]].map(([h,f]) => <th key={f} style={{ padding: "13px 9px", textAlign: "right", whiteSpace: "nowrap", border: "1px solid rgba(255,255,255,.55)", fontSize: "13px", fontWeight: 900 }}><button type="button" onClick={() => togglePurchaseSort(f)} style={{ border: 0, background: "transparent", color: "white", cursor: "pointer", font: "inherit", width: "100%", textAlign: "right" }}>{h} <span style={{ color: "#ccfbf1", fontSize: "15px" }}>{sortArrow(f)}</span></button></th>)}</tr></thead><tbody>{visibleItems.length ? visibleItems.map((r, i) => <tr key={r.id || i} style={{ borderBottom: "1px solid #cbd5e1", background: i % 2 === 0 ? "#ffffff" : "#f8fafc" }}><td style={{ padding: "10px 8px", fontWeight: 900, border: "1px solid #cbd5e1", verticalAlign: "middle" }}>{itemRequestNo(r) || "-"}</td><td style={{ padding: "10px 8px", border: "1px solid #cbd5e1", verticalAlign: "middle" }}>{r.item_name || r.name || "-"}</td><td style={{ padding: "10px 8px", border: "1px solid #cbd5e1", verticalAlign: "middle" }}>{r.unit || "-"}</td><td style={{ padding: "10px 8px", border: "1px solid #cbd5e1", verticalAlign: "middle" }}>{r.quantity_requested ?? "-"}</td><td style={{ padding: "10px 8px", border: "1px solid #cbd5e1", verticalAlign: "middle" }}>{r.quantity_executed ?? "-"}</td><td style={{ padding: "10px 8px", border: "1px solid #cbd5e1", verticalAlign: "middle" }}>{r.quantity_remaining ?? "-"}</td><td style={{ padding: "10px 8px", border: "1px solid #cbd5e1", verticalAlign: "middle" }}>{r.requesting_department || "-"}</td><td style={{ padding: "10px 8px", border: "1px solid #cbd5e1", verticalAlign: "middle" }}>{r.executor || "-"}</td></tr>) : <tr><td colSpan={8} style={{ padding: 30, textAlign: "center", color: "#64748b" }}>{selectedRequest ? "لا توجد أصناف مرتبطة بهذا الطلب في جدول الأصناف" : "لم يتم تحديد طلب بعد"}</td></tr>}</tbody></table></div>
    </>}
  </div>;
});
...
function AdminAffairsSystemInner() {
  ...
  useEffect(() => {
    // Restore the session once. Login no longer performs an owner-bootstrap
    // round-trip, so this path stays limited to session and role resolution.
    let mounted = true;
    ...
    const resolveUser = async (session: any) => {
      ...
      try {
        ...
        const roleSet = new Set((roleResult.data || []).map((r: any) => r.role));
        ...
        if (managerResult.data) {
          name = managerResult.data.name || name;
          permissions = managerResult.data.permissions || undefined;
        }
        if (!mounted) return;
  ...
  if (!isSupabaseConfigured) {
    return (
      <div className="auth-state-screen" dir="rtl">
        <div className="auth-state-card">
          <ShieldCheck size={34} className="text-emerald-700" />
          <h1>النظام جاهز تقريبًا</h1>
          <p>أضف متغيري <code>VITE_SUPABASE_URL</code> و<code>VITE_SUPABASE_PUBLISHABLE_KEY</code> إلى بيئة التشغيل لتفعيل تسجيل الدخول والبيانات.</p>
          <p className="auth-state-hint">راجع ملف <code>.env.example</code> المرفق ثم أعد تشغيل الخادم.</p>
        </div>
      </div>
    );
  }
  ...
  if (authChecking) {
    return (
      <div className="auth-state-screen" dir="rtl">
        <div className="auth-state-card auth-state-card--loading">
          <Loader2 size={34} className="animate-spin text-emerald-700" />
          <h1>جارٍ تجهيز الجلسة</h1>
          <p>لحظات ونفتح لك النظام بأمان.</p>
        </div>
      </div>
    );
  }
